...
public class ProductServiceImpl implements ProductService {
    ...
    public ProductResponse getByProductCode(String productCode) {
        ...
        //2.获取商品的图片
        List<String> productImages = productImageMapper.listByProductCode(productCode);
        //3.拼接图片host
        ...
        //4获取商品sku信息
        List<SkuResponse> skuResponses = skuMapper.listByProductCode(productCode);
        //5获取商品sku对应的库存
        List<SkuStockResponse> skuStockResponseList = skuStockMapper.listByProductCode(productCode);
        ...
        //7获取商品sku对应的价格
        List<SkuPriceResponse> skuPriceResponses = skuPriceMapper.listByProductCode(productCode);
    ...
    public ProductResponse getByProductCode6(String productCode) throws CloneNotSupportedException, InterruptedException {
        Map<String, Object> withMultiTypeTask = AsyContextCallable.submitWithMultiTypeTask(executor,
                ...
                new MultiTaskCallable("image") {
                    @Override
                    public Object call() throws Exception {
                        return productImageService.listByProductCode(productCode);
                    }
                },
                new MultiTaskCallable("sku") {
                    @Override
                    public Object call() throws Exception {
                        return skuService.listByProductCode(productCode);
                    }
                });
        ProductResponse productResponse= (ProductResponse) withMultiTypeTask.get("product");
